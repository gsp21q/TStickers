...
st.subheader("حوّل أي فيديو إلى صيغة .WEBM المتوافقة رسميًا بثوانٍ معدودة")

uploaded_file = st.file_uploader("اختر ملف فيديو (MP4, MOV, MKV)", type=["mp4", "mov", "mkv", "avi"])
...
if uploaded_file is not None:
    ...
    if st.button("🚀 ابدأ المعالجة والتحويل"):
        if selected_duration > 3.0:
            ...
            with st.spinner("جاري معالجة الفيديو وضغطه بكوديك VP9... قد يستغرق هذا دقيقة"):
                output_filename = "telegram_target.webm"
                if os.path.exists(output_filename):
                    os.remove(output_filename)
                
                # Intelligent Bitrate calculation to guarantee size < 256KB
                # Max safe bitrate = (250 KB * 8) / duration
                safe_bitrate_kbps = math.floor((240 * 8) / selected_duration)
                
                # FFmpeg script building
                # -an: No audio, -c:v libvpx-vp9: VP9 codec, -r 30: 30 FPS maximum
                ffmpeg_cmd = (
                ...
                if os.path.exists(output_filename) and os.path.getsize(output_filename) > 0:
                    ...
                    st.markdown('<div class="success-box">🎉 تم تحويل وتجهيز الفيديو بنجاح ومطابقة كافة الشروط المحددة!</div>', unsafe_allow_html=True)
                    ...
                    st.error("فشلت عملية التحويل. يرجى التحقق من صياغة الفيديو الأصلي.")
